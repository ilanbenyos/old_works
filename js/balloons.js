...
//var audioPop = new Audio('sounds/pop.wav');
...
function generBal(){
    ...
    var randBottom = Math.floor (Math.random() * (gScreenHeight/3));
    //var randBottom = 0;
    var randRadius = Math.floor(gMinBalRad + Math.random() * (gMaxBalRad - gMinBalRad));
    var randSpeed = 2*Math.random() ;
...
function moveBals() {
    for(var i in gBals){
        ...
        //balloon above screen
        if (balEl.bottom > gScreenHeight){   //balloon out of screen
            balEl.boom = true;
        }
        //ballon is boom - by user or by pos
        if (balEl.boom === true){
